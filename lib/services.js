...
MediatorPanel.prototype = {
  ...
  observe: function(document, aTopic, aData) {
    let tb = this.window.document.getElementById('activities-tabbrowser-'+this.methodName);
  ...
  inject: function(document, tab) {
    ...
    tab.worker.port.on('activity.result', this.onActivitySuccess.bind(this));
    tab.worker.port.on('activity.error', this.onActivityFailure.bind(this));
    let self = this;
  ...
  onPanelHidden: function() {
    ...
    this.panel.port.once("activity.mediation.setMediatorState", function(state) {
      activityToUpdate.mediatorState = state;
    });
    this.panel.port.emit("activity.mediation.panelHidden");
  },
  ...
  _createPanelOverlay: function() {
    ...
    panel.setAttribute('id', 'activities-panel-'+this.methodName);
    ...
    tabs.setAttribute('id', 'activities-tabs-'+this.methodName);
    tabs.setAttribute('tabbrowser', 'activities-tabbrowser-'+this.methodName);
    ...
    tb.setAttribute('id', 'activities-tabbrowser-'+this.methodName);
    tb.setAttribute('type', 'content');
    tb.setAttribute('class', 'activities-tabbrowser');
    tb.setAttribute('flex', '1');
    tb.setAttribute('tabcontainer', 'activities-tabs-'+this.methodName);
  ...
  _createPopupPanel: function() {
    ...
    this.panel = window.document.getElementById('activities-panel-'+this.methodName);
    let tb = window.document.getElementById('activities-tabbrowser-'+this.methodName);
  ...
  show: function() {
    if (this.invalidated) {
      ...
      //this.panel.port.emit("activity.mediation.updateActivity", this.tabData.activity);
