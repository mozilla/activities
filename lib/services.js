...
MediatorPanel.prototype = {
  ...
  newInjector: function(document, tab) {
    if (!tab.service) {
    ...
    tab.worker =  Worker({
      ...
      contentScriptFile: tab.service.app.contentScriptFile
    });
  ...
  attachHandlers: function() {
    console.log("*** FIXME handlers are not attached");
    return;
  ...
  _createPopupPanel: function() {
    ...
    //this.panel = thePanel;
    this.panel = this.window.document.getElementById('activities-panel');
    activityRegistry.get(this.methodName, function(serviceList) {
      let tb = this.window.document.getElementById('activities-tabbrowser');
